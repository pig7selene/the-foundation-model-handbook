...
These instructions govern every new standalone chapter in this repository. Chapters 1–39, `docs/style-guide.md`, `docs/chapter-template.md`, the shared Typst files, and the v1.0 book manifest in `main.typ` are the project baseline. Do not begin a new chapter by copying generic Foundation Model prose or by inventing a separate visual style.
...
4. Inspect the current chapter source structure and the matching build filename before creating a new directory. If the chapter belongs in an assembled edition, also inspect the reading order declared in `main.typ`.
...
8. Do not alter the v1.0 book manifest to add a topic outside the release scope. A later edition requires an explicit roadmap and manifest update.
...
6. When a change affects the assembled edition, build it with `./scripts/build-release.sh` and inspect the relevant front matter, part or chapter boundary, and affected pages in addition to the standalone PDF.
...
4. Confirm that the source, bibliography, README link, outline entry, generic `.gitignore` tracked-PDF rule, and `build/part-name/NN-topic-slug.pdf` agree on the chapter number, slug, and title. When applicable, verify the `main.typ` inclusion and release-PDF path as well.
