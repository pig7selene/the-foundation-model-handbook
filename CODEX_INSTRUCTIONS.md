...
These instructions govern every new standalone chapter in this repository. Chapters 1–6, `STYLE_GUIDE.md`, `CHAPTER_TEMPLATE.md`, and the shared Typst files are the project baseline. Do not begin a new chapter by copying generic Foundation Model prose or by inventing a separate visual style.
...
3. Read `STYLE_GUIDE.md`, `CHAPTER_TEMPLATE.md`, `BOOK_OUTLINE.md`, `README.md`, `template/chapter.typ`, `template/typography.typ`, `template/environments.typ`, `template/notation.typ`, and `references/references.bib`.
...
1. Confirm the chapter's place in `BOOK_OUTLINE.md`, its prerequisites, and its exclusions.
...
3. Add complete, clean entries to `references/references.bib`, using DOI fields when available and stable direct URLs otherwise.
...
1. Create `chapters/NN-topic-slug/main.typ` and import the existing shared Typst template. Do not duplicate the template locally.
...
3. Follow the logical structure in `CHAPTER_TEMPLATE.md`, adapting it to the subject rather than padding it.
...
1. Build the standalone PDF:
   ...
   ```bash
   typst compile --root . chapters/NN-topic-slug/main.typ build/NN-topic-slug.pdf
   ```
...
1. Compare terminology and notation with Chapters 1–6 and `template/notation.typ`.
...
4. Confirm that the source, bibliography, README link, outline entry, `.gitignore` exception, and `build/NN-topic-slug.pdf` agree on the chapter number, slug, and title.
