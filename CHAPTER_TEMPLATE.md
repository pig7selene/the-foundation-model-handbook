...
// #import "../../template/environments.typ": definition, algorithm
...
Create a chapter directory only when the chapter is ready to be researched and written:

```text
chapters/NN-topic-slug/
└── main.typ
```
...
Version the generated PDF in `build/`, add its explicit exception to `.gitignore`, add a README link under the correct topic group, and update `BOOK_OUTLINE.md` once the chapter is complete.
