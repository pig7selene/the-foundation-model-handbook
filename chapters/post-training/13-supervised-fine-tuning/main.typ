...
Implementations commonly scale the update by $alpha / r$ and initialize one factor so that $Delta W = 0$ at the start, preserving the base function before training. During the forward pass, the layer computes $W_0 x + B (A x)$; after training, the update can be stored separately or merged into $W_0$ for deployment. Rank, scaling, target modules, bias treatment, and merge convention are part of the adapter specification. Chapter 14 gives the broader PEFT treatment.
