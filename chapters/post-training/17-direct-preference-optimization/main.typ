...
The final term depends on the prompt but not on which candidate response is compared. The Bradley--Terry model from Chapter 15 uses only a reward difference. For two candidates, the prompt-only term in @eq-dpo-implicit-reward-reparameterization cancels:
